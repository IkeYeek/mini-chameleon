\section{Sequential DGEMM implementation} % This adds it to the Table of Contents automatically
...
\begin{frame}{First implementations}
    ...
    \end{block}
\end{frame}
\begin{frame}{Research on efficient GEMM}
    Most of recent work concerns parallel GEMM or mixed-precision GEMM.\\
...
\begin{frame}{GOTO}
    ...
    \only<2>{
        \begin{block}{GEMM}
            \includegraphics[width=.7\textwidth, keepaspectratio]{assets/goto_1.png}
        \end{block}
    }
    \only<3>{
        \begin{block}{GEPP}
            \includegraphics[width=.8\textwidth, keepaspectratio]{assets/goto_2.png}
        \end{block}
    }
    \only<4>{
        \begin{block}{GEBP}
            \includegraphics[width=.9\textwidth, keepaspectratio]{assets/goto_3.png}
        \end{block}
    }
    \only<5>{
    ...
    \only<6>{
        \begin{block}{Microkernel}
            \includegraphics[width=1\textwidth, keepaspectratio]{assets/goto_5.png}
        \end{block}
    }
    \only<7>{
        \begin{block}{Microkernel Implementation}
            \vspace*{1pt}
            \includegraphics[width=1\textwidth, keepaspectratio]{assets/goto_microkernel.png}
        \end{block}
        \begin{itemize}
            \item Uses 10 out of the 16 available vectorial registers.
            \item Heavy loop unrolling to avoid conditional jumps.
            \item MR = 4, NR = 8.
        \end{itemize}
    }
    \only<8-9>{
        \begin{block}{Results for square matrices ($M = N = K$)}<only@8>
            \includegraphics[width=1\textwidth, keepaspectratio]{assets/goto_vs_vendor.png}
        \end{block}
        \begin{block}{Impact of Goto on memory accesses}<only@9>
            \includegraphics[width=1\textwidth, keepaspectratio]{assets/goto_vtune_mem.png}
        \end{block}
    }
    \only<10>{
        \begin{block}{Conclusion}
        \begin{itemize}
            \item We managed to get from 16GFlop/s peeks to up to 25GFlop/s, reaching up to 75\% of OpenBLAS speed.
            \item We are now bounded by L1 cache accesses. While that's better, that means there are still pace for ameliorations, probably with even better cache management and handwritten assembly.
