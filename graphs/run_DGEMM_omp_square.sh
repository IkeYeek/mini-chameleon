...
VARIANTS="goto omp omp-t"
SIDE_SIZES="256 512 1024 2048 4096 8192"
THREADS=" 4 8 12 24"
ITER=5
      ...
      if need_run "$FILE/$OUTPUT_FILE" $((ITER + 1)); then
        ...
        for bs in $BLOCK_SIZES; do
          for threads in $THREADS; do
            ...
            if need_run "$FILE/$OUTPUT_FILE" $((ITER + 1)); then
              OMP_NUM_THREADS=$threads SEQ_VER=goto $TEST_PATH -v $var -i $ITER -M $size -N $size -K $size -b $bs > "$FILE/$OUTPUT_FILE"
            else
              echo "Skipping experiment (output file exists and complete)"
            fi
          done
