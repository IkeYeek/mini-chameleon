    ...
    if [ "$var" = "seq" ]; then
      ...
      if need_run "$FILE/$OUTPUT_FILE" $((ITER + 1)); then
        ...
        for threads in $THREADS; do
          ...
          OUTPUT_FILE="getrf-omp_t-${size}x${size}x${size}-${threads}_threads_bs_${BLOCK_SIZES}.raw"

          if need_run "$FILE/$OUTPUT_FILE" $((ITER + 1)); then
            OMP_NUM_THREADS=$threads SEQ_VER=goto $TEST_PATH -v $var -i $ITER -M $size -N $size -b $BLOCK_SIZES > "$FILE/$OUTPUT_FILE"
          else
            echo "Skipping experiment (output file exists and complete)"
          fi
        done
...
for file in *.raw; do
  ...
  python3 ../formatter_dgetrf.py -i "$file" -al -o "getrf_omp-t_square.out"
done
for file in *.out; do
  echo "Processing text file: $file"
