...
for size in $SIDE_SIZES; do
  for var in $VARIANTS; do
    ...
    if [ "$var" = "goto" ]; then
      echo "Launching: Variant=$var, MxNxK=${size}x${size}x${size}, ITER=$ITER"
      ...
      if need_run "$FILE/$OUTPUT_FILE" $((ITER + 1)); then
        ...
        for threads in $THREADS; do
          echo "Launching: Variant=OMP/MPI, MxNxK=${size}x${size}x${size}, ITER=$ITER, THREADS/PROCESSES=$threads"
          OUTPUT_FILE_OMP="gemm-OMP-${size}x${size}x${size}-t-${threads}.raw"
          OUTPUT_FILE_MPI="gemm-MPI-${size}x${size}x${size}-p-${threads}.raw"

          
          if need_run "$FILE/$OUTPUT_FILE_OMP" $((ITER + 1)); then
            OMP_NUM_THREADS=$threads SEQ_VER=goto $TEST_PATH -v $var -i $ITER -M $size -N $size -K $size > "$FILE/$OUTPUT_FILE_OMP"
          else
            echo "Skipping experiment (output file exists and complete)"
          fi

          if need_run "$FILE/$OUTPUT_FILE_MPI" $((ITER + 1)); then
            OMP_NUM_THREADS=1 MPI_VER=summa mpirun --np $threads --mca mtl psm2 $TEST_PATH -v mpi -i $ITER -M $size -N $size -K $size > "$FILE/$OUTPUT_FILE_MPI"
          else
            echo "Skipping experiment (output file exists and complete)"
          fi
