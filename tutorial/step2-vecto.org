...
Enable it in =mini-chameleon= with ~-DMIPP=ON~. Assuming being at the
root of =mini-chameleon=:

#+begin_src bash
  mkdir -p build/mipp
  cmake . -B build/mipp -DENABLE_MIPP=ON
...
~gemm_mipp.cpp~ buffer (~M-x c++-mode~ ~M-x rmsbolt-mode~). You can
then just press ~C-c C-c~ each time you want to (re-)generate the
assembly code corresponding to your current code. As a reminder, you
can run ~emacs-bedrock~ with:
