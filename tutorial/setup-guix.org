...
*** Avec un compilateur récent (utile pour la vectorisation p. ex.)
     ...
     Par défaut =guix= fait le choix relativement conservateur d'utiliser =gcc 10.3=
     afin de garantir le support d'un maximum de paquets. Il est possible de
     choisir un compilateur plus récent (=gcc 12.2=) en chargeant explicitement
     ~gcc-toolchain~:
  ...
  #+end_src
     ...
     On peut choisir un compilateur alternatif, p. ex. =clang 14.0.6= (plus
  ...
  #+end_src
     ...
     Notez que les paquets installés restent compilés avec =gcc 10.3= et qu'il
...
**** Utiliser l'option ~--tune~ de guix

=guix= peut automatiquement tuner votre code selon l'architecture avec l'option ~--tune~:
