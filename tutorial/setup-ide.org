...
can be accessed with =F10=. You can for instance press =F10= and
...
you prefer ~vim~-like key bindings, you can add ~(require 'evil)~ in
your =emacs= init file (typically ~~/.config/emacs/init.el~ or
~~/.emacs.d/init.el~) or a more fancy configuration such as the
[[https://gitlab.inria.fr/solverstack/mini-examples/mini-chameleon/-/blob/master/.dir-locals.el][~init-evil.el~]] provided at the root of the =mini-chameleon= project.

Note that it is possible to permanently install ~emacs-bedrock~ with
~guix install emacs-bedrock-as-default~. In this case, you may want to
use the [[https://gitlab.inria.fr/solverstack/mini-examples/mini-chameleon/-/blob/master/.bash_profile][~.bash_profile~]] file at the root of the =mini-chameleon=
project as (or within) your own ~~/.bash_profile~. ~emacs-bedrock~ can
then simply be launched with the ~emacs~ command.
