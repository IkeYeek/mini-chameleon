...
#+begin_src bash
  guix shell --pure emacs-bedrock-as-default -D mini-chameleon clang-toolchain -- emacs -nw -f xterm-mouse-mode
#+end_src
...
+support]] with ~-f xterm-mouse-mode~. It is also possible to access
the top [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Menu-Bar.html][emacs menu]] with =F10=. You can for instance press =F10= and
=Enter= to visit a new file. If the =F10= shortcut is aleady taken by
