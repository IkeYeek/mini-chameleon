...
int dgemm_omp_naive(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE transA,
...
int dgemm_seq_scalaire(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE transA,
                       CBLAS_TRANSPOSE transB, const int M, const int N,
                       const int K, const double alpha, const double *A,
                       const int lda, const double *B, const int ldb,
                       const double beta, double *C, const int ldc) {
...
static inline void dgepp(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE transA,
  ...
  if (m < M) {
    int Mb = M - m;
    dgemm_seq_scalaire(layout, transA, transB, Mb, N, K, alpha, A_panel + m,
                       lda, B_panel, ldb, 1.0, C + m, ldc);
  }
}
