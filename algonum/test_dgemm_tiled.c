 ...
 *
 * @param[in] dplrnt
 *          The function pointer to a dplrnt operation on tiled format to
...
int testone_dgemm_tiled(dplrnt_tiled_fct_t dplrnt, dgemm_tiled_fct_t dgemm,
  ...
  perf(&start);
  rc = dgemm(CblasColMajor, transA, transB, M, N, K, b, alpha,
             (const double **)Atile, (const double **)Btile, beta, Ctile);
  perf(&stop);

  if (rc == ALGONUM_NOT_IMPLEMENTED) {
    if (global_options.mpirank == 0) {
      printf("tA=%s tB=%s M= %4d N= %4d K= %4d: " ALGONUM_COLOR_ORANGE
  ...
  if (check) {
    ...
    if (global_options.mpirank == 0) {
      ...
      rc = check_dgemm(transA, transB, M, N, K, alpha, A, lda, B, ldb, beta,
                       Cinit, C, ldc);

      if (rc == ALGONUM_SUCCESS) {
        printf("tA=%s tB=%s M= %4d N= %4d K= %4d alpha= %e beta= %e: %le "
               "GFlop/s: " ALGONUM_COLOR_GREEN "SUCCESS\n" ALGONUM_COLOR_RESET,
               (transA == CblasNoTrans) ? "NoTrans" : "Trans",
               (transB == CblasNoTrans) ? "NoTrans" : "Trans", M, N, K, alpha,
               +beta, gflops);
      } else {
        printf("tA=%s tB=%s M= %4d N= %4d K= %4d alpha= %e beta= %e: %le "
               "GFlop/s: " ALGONUM_COLOR_RED "FAIL\n" ALGONUM_COLOR_RESET,
               (transA == CblasNoTrans) ? "NoTrans" : "Trans",
               (transB == CblasNoTrans) ? "NoTrans" : "Trans", M, N, K, alpha,
               +beta, gflops);
      }
    ...
    if (global_options.mpirank == 0) {
      printf("tA=%s tB=%s M= %4d N= %4d K= %4d: %le GFlop/s\n",
             (transA == CblasNoTrans) ? "NoTrans" : "Trans",
             (transB == CblasNoTrans) ? "NoTrans" : "Trans", M, N, K, gflops);
    }
  }
 ...
 *
 */
int testall_dgemm_tiled(dplrnt_tiled_fct_t dplrnt, dgemm_tiled_fct_t dgemm) {
        ...
        for (in = 0; in < nb_N; in++) {
          ...
          for (ik = 0; ik < nb_K; ik++) {
            ...
            for (ib = 0; ib < nb_b; ib++) {
              ...
              rc = testone_dgemm_tiled(dplrnt, dgemm, tA, tB, m, n, k, b, 1);
              if (rc == ALGONUM_FAIL) {
                nbfailed++;
