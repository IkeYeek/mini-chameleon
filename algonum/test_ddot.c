...
double flops_ddot(int N) { return flops_dgemm(1, 1, N); }
  ...
  *C = ddot(N, A, lda, B, ldb);
  ...
  if (check) {
    ...
    if (rc == ALGONUM_SUCCESS) {
      fprintf(fd,
              "N= %4d: %le GFlop/s: " ALGONUM_COLOR_GREEN
              "SUCCESS\n" ALGONUM_COLOR_RESET,
              N, gflops);
    } else {
      fprintf(fd,
              "N= %4d: %le GFlop/s: " ALGONUM_COLOR_RED
              "FAIL\n" ALGONUM_COLOR_RESET,
              N, gflops);
    }
    free(Cinit);
  } else {
    fprintf(fd, "N= %4d: %le GFlop/s\n", N, gflops);
  }
...
int testone_ddot(ddot_fct_t ddot, int N, int check) {
  return testone_ddot_real(ddot, N, check, stdout);
}
