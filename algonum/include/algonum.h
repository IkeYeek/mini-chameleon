 ...
 */
static inline int my_iceil(int a, int b) { return (a + b - 1) / b; }

/**
 * Helper function to compute integer min
 */
static inline int my_imin(int a, int b) { return (a < b) ? a : b; }

/**
 * Helper function to compute integer max
 */
static inline int my_imax(int a, int b) { return (a < b) ? b : a; }
 ...
 */
void CORE_dplrnt(double bump, int m, int n, double *A, int lda, int bigM,
                 int m0, int n0, unsigned long long int seed);
void dplrnt_tiled(double bump, int M, int N, int b, double **A,
                  unsigned long long int seed);
void dplrnt_tiled_starpu(double bump, int M, int N, int b, double **A,
                         unsigned long long int seed);
...
typedef int (*dgemm_fct_t)(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE transA,
                           CBLAS_TRANSPOSE transB, int M, int N, int K,
                           double alpha, const double *A, int lda,
                           const double *B, int ldb, double beta, double *C,
                           int ldc);

typedef int (*dgemm_tiled_fct_t)(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE transA,
                                 CBLAS_TRANSPOSE transB, int M, int N, int K,
                                 int b, double alpha, const double **A,
                                 const double **B, double beta, double **C);

typedef int (*dgetrf_fct_t)(CBLAS_LAYOUT layout, int m, int n, double *A,
                            int lda);

typedef int (*dgetrf_tiled_fct_t)(CBLAS_LAYOUT layout, int m, int n, int b,
                                  double **A);
 ...
 */
int check_ddot(const int N, const double *X, const int incX, const double *Y,
               const int incY, double *res_ref, const double *res);
int testone_ddot(ddot_fct_t ddot, int N, int check);
int testone_ddot_real(ddot_fct_t ddot, int N, int check, FILE *fd);
 ...
 */
int check_dgemm(CBLAS_TRANSPOSE transA, CBLAS_TRANSPOSE transB, int M, int N,
                int K, double alpha, const double *A, int lda, const double *B,
                int ldb, double beta, double *Cref, const double *C, int ldc);
int testone_dgemm(dgemm_fct_t dgemm, CBLAS_TRANSPOSE transA,
                  CBLAS_TRANSPOSE transB, int M, int N, int K, int check);
int testall_dgemm(dgemm_fct_t tested_dgemm);
int testwarm_dgemm(dgemm_fct_t dgemm, CBLAS_TRANSPOSE transA,
                   CBLAS_TRANSPOSE transB, int M, int N, int K);

/**
 * Testing functions for the LU factorization in LAPACK layout
 */
int check_dgetrf(int M, int N, double *LU, double *A, int lda);
int testone_dgetrf(dgetrf_fct_t dgetrf, int M, int N, int check);
 ...
 */
typedef void (*dplrnt_tiled_fct_t)(double bump, int M, int N, int b, double **A,
                                   unsigned long long int seed);

int testone_dgemm_tiled(dplrnt_tiled_fct_t dplrnt, dgemm_tiled_fct_t dgemm,
                        CBLAS_TRANSPOSE transA, CBLAS_TRANSPOSE transB, int M,
                        int N, int K, int b, int check);
 ...
 */
int testone_dgetrf_tiled(dplrnt_tiled_fct_t dplrnt, dgetrf_tiled_fct_t dgetrf,
 ...
 */
int testone_dgetrf_cuda(dgetrf_fct_t dgetrf, int M, int N, int check);
int testone_dgemm_cuda(dgemm_fct_t dgemm, CBLAS_TRANSPOSE transA,
                       CBLAS_TRANSPOSE transB, int M, int N, int K, int check);
