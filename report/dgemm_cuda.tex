...
Nous avons eu en cette derniere année de cours des enseignements de programmation GPU sur la plateforme CUDA\cite{cuda_toolkit} (développée par NVIDIA, plus grand vendeur de cartes graphiques au monde).

Nous avons choisi d'implémenter une version CUDA du DGEMM dans le cadre de ce projet.
L'objectif initial était d'arriver à battre les performances de l'implémentation officielle de NVIDIA faisant partie de CuBLAS\cite{cublas_library},
...
Pour ce faire, nous avons dans un premier temps implémenté un DGEMM "naïf" pour A100, puis nous avons itérativement raffiné notre noyau jusqu'à atteindre des performances comparables à CuBLAS.

Pour toutes les implémentations CUDA du projet, nous avons utilisé la bibliothèque CuTe (faisant partie de CUTLASS\cite{Thakkar_CUTLASS_2023}) de NVIDIA.
...
que les coeurs de tenseur (unités matérielles ne servant qu'à accélérer les GEMM) doivent être utilisés pour espérer atteindre les performances maximales annoncées.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/a100_specs.png}
    \caption{Spécifications de la carte A100 de NVIDIA (Figure tirée du papier "NVIDIA A100 Tensor Core GPU Architecture"\cite{nvidia2020ampere})}
    \label{fig:a100-specs}
\end{figure}

Nous avons donc choisi d'implémenter le DGEMM le plus minimal possible utilisant les tensor core double de l'architecture AMPERE (instruction \texttt{mma.sync.aligned.m8n8k4.row.col.f64.f64.f64.f64}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/m8n8k4.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/cuda_baseline.png}
    \caption{Résultats de notre implémentation naïve (baseline) contre CuBLAS et la performance maximum théorique sur A100 SXM4.}
    \label{fig:baseline}
\end{figure}


\subsection{Première optimisation : agrandissement du micronoyau}
Actuellement, notre micronoyau fait exactement la taille de notre tensor core (8x8x4).
Pour augmenter notre intensité arithmétique, nous avons "agrandi" notre micronoyau en apellant deux fois le tensor core sur l'axe K.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/m8n8k8.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/cuda_baseline_m8n8k8.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/128b_cpy.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/cuda_baseline_m8n8k8_128b.png}
...
Pour se faire, on peut dupliquer notre microkernel 8 fois sur \texttt{M} et \texttt{N} et 8 fois sur \texttt{K} \footnote{CuBLAS utilise la même taille de bloc que nous \textbf{(64,64,32)}.}.
(pour trouver ces nombres nous avons fait des expériences avec une multitude de tailles différentes.)

On peut voir sur la Figure \ref{fig:m64n64k32} en annexe la taille des blocs A et B ainsi que du résultat C calculé par chaque WARP (et bloc pour l'instant puisqu'un bloc = 1 WARP).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/cuda_baseline_m64n64k32_128b.png}
...
En observant le papier officiel sur la microarchitecture Ampere ("NVIDIA A100 Tensor Core GPU Architecture"),
...
Sur la Figure \ref{fig:m64n64k32_4W} en annexe, on remarque que l'on utilise maintenant 128 threads pour exécuter notre microkernel.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/cuda_baseline_m64n64k32_128b_4W.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.48\textwidth]{assets/dgemm_cuda/cuda_baseline_m64n64k32_128b_4W_CG.png}
...
Les optimisations basées sur une découverte dynamique du L2 (fragmenté en 2 parties, voir annexe \ref{appendix:l2split}) ne semblent pas non plus faites.
