...
% https://github.com/Leikoe/ampere-L2-partitions-microbenchmarks
...
En 2021, des chercheurs de chez Citadel ont donné une presentation au GTC de NVIDIA où ils ont montré qu'il existait une très grande pénalité de performance lors des accès à la "mauvaise" partie du L2 de la carte graphique depuis un SM.
Cette mauvaise partie est enfaite la partition la plus loin du SM.
Les SMs sont disposés autour des deux partitons du L2 et lorsqu'ils veulent accéder à celle dont ils ne sont pas collés, ils doivent passer par l'interconnect des deux partitions du L2.
...
Nous avons reproduit ces benchmarks dans le cadre du projet (voir figures \ref{fig:l2sm0} et \ref{fig:l2sm2}) en espérant pouvoir intégrer une optimisation en tirant partie à notre GEMM CUDA.
\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{assets/dgemm_cuda/l2_latency_vs_offset_smid0.png}
        \caption{Graphique des latences engendrées par un accès en fonction de l'offset dans le L2.}
        \label{fig:l2sm0}
    \end{minipage}
     \hfill
    \begin{minipage}{0.48\textwidth}
